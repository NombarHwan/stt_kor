...
def _preload_nvidia_dlls() -> None:
    ...
    # immediately instead of searching. On machines without an NVIDIA GPU
    # (or without the nvidia-* pip packages installed), the import below
    # simply fails and this is a no-op.
    if os.name != "nt":
        return
    try:
        import ctypes
...
# faster-whisper (CTranslate2) only accelerates on NVIDIA CUDA GPUs — AMD
# cards (ROCm) aren't supported by the backend, no matter what's detected
# here. "auto" picks CUDA when an NVIDIA GPU is available and transparently
# falls back to CPU otherwise (including on AMD machines). Override with the
# STT_DEVICE / STT_COMPUTE_TYPE env vars if you want to force specific
# values, e.g. `STT_DEVICE=cpu STT_COMPUTE_TYPE=int8` for a faster CPU run.
...
def print_usage() -> None:
    print("사용법: python stt_korean_0.0.1.py <검색어 또는 파일명> [검색어2 ...]")
    print("  - 녹음 폴더 안의 파일명에 검색어가 포함되면 모두 변환 대상이 됩니다.")
    print("  - 예: python stt_korean_0.0.1.py 0901")
    print("  - 예: python stt_korean_0.0.1.py 정치와공론 컴퓨터구조")
    print()
